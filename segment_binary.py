...
def segment(input_image_path, output_mask_path):
    ...
    # START PARTICIPANT CODE
    # ===============================================
    from mmsegmentation.demo.inference_ import image_inference
    ...
    # checkpoint = "segnext_binary_iter_20000.pth"
    config = "segnext_mscan-t_1xb16-adamw-40k_syniss_binary-512x512.py"
    checkpoint = "segnext_binary_iter_20000.pth"
    result = image_inference(input_image_path, config, checkpoint)
    # print("result.pred_sem_seg======",result.pred_sem_seg)
    pred_labels = result.pred_sem_seg.cpu().data[0].numpy()
    # print("=======pred_labels shape =====", pred_labels.shape)
    # print("pred_labels===========", type(pred_labels.pred_sem_seg.cpu().data))
